...
with tab1:
    if response_time_by_day:
        df_response = pd.DataFrame(response_time_by_day, columns=['Date', 'Temps moyen (s)'])
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate='<b>%{x|%d/%m/%Y}</b><br>Temps: %{y:.2f}s<extra></extra>'
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate='Objectif: 5.00s<extra></extra>'
        ))
        ...
        with col1:
            st.metric("⚡ Temps moyen", f"{df_response['Temps moyen (s)'].mean():.2f}s")
        with col2:
            st.metric("📈 Maximum", f"{df_response['Temps moyen (s)'].max():.2f}s")
        with col3:
            st.metric("📉 Minimum", f"{df_response['Temps moyen (s)'].min():.2f}s")
    else:
        st.info(
