...
def register_user_admin(matricule, nom, prenom, email, password, role):
    """Enregistre un nouvel utilisateur (fonction admin - garde l'ancienne signature)"""
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def register_user(matricule: str, nom: str, prenom: str, email: str, password: str) -> tuple[bool, str]:
    """
    ...
    # Validation de la longueur du matricule
    if len(matricule) < 4:
        return False, "Le matricule doit contenir au moins 4 caractères"
    ...
    try:
        with get_connection() as conn:
            ...
            cursor.execute("SELECT matricule FROM users WHERE matricule = %s", (matricule,))
            if cursor.fetchone():
                cursor.close()
                return False, "Ce matricule existe déjà"
            ...
            cursor.execute("""
                INSERT INTO users (matricule, nom, prenom, email, password, role)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (matricule, nom.strip(), prenom.strip(), email.lower(), hashed_password, 'rh'))
        ...
        if 'unique constraint' in str(e).lower():
            if 'email' in str(e).lower():
                return False, "Cet email est déjà utilisé"
            elif 'matricule' in str(e).lower():
                return False, "Ce matricule existe déjà"
        return False, "Erreur : données en conflit"
    except Exception as e:
...
# new
def update_user_password(matricule: str, old_password: str, new_password: str) -> tuple[bool, str]:
    """
    ...
    try:
        with get_connection() as conn:
            ...
            cursor.execute(
                "SELECT password FROM users WHERE matricule = %s",
                (matricule,)
            )
            row = cursor.fetchone()
            ...
            cursor.execute(
                "UPDATE users SET password = %s WHERE matricule = %s",
                (hashed_new_password, matricule)
            )
            conn.commit()
            cursor.close()
